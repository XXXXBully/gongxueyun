...
- **管理端：** 管理用户和租户、查看审计日志、配置通知、批量执行任务、测试 AI 与地理编码能力，支持为指定用户刷新缺卡记录、手动补卡和补交报告。
...
- **管理端能力完成：** 支持后台登录、角色权限、用户管理、租户管理、用户执行日志、审计日志查询与清空、通知配置、SMTP 测试、AI 测试、地理编码搜索与逆地理解析。
...
- 工学云补卡代理也可以在管理端 Web 的「系统设置」中按租户配置。环境变量中的 `MOGUDING_PROXY_API_URL` / `MOGUDING_PROXY_URLS` 优先级高于 Web 配置；非默认租户的 Web 配置不会覆盖 `default` 租户。
...
- `/ai/test` 和正式 AI 生成链路默认都拒绝本机、内网、链路本地和特殊地址；正式链路如需访问内网模型服务，必须同时打开 `ALLOW_PRIVATE_AI_ENDPOINTS=true` 并配置 `AI_ALLOWED_HOSTS`。
